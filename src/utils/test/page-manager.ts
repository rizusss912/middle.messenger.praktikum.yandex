...
export class PageManager {
    ...
    appendChildToBody(tag: string): Promise<void> {
        return this.page.evaluate(tag => {document.body.appendChild(document.createElement(tag))}, tag);
    }
    ...
    hasAttribute(selector: string, attributeName: string): Promise<boolean> {
        return this.page.evaluate(
            (selector, attributeName) => Boolean(document.body.querySelector(selector)?.hasAttribute(attributeName)),
            selector,
            attributeName,
        );
    }
    ...
    getAttribute(selector: string, attributeName: string): Promise<string | null> {
        return this.page.evaluate(
            (selector, attributeName) => document.body.querySelector(selector)?.getAttribute(attributeName),
            selector,
            attributeName,
        );
    }

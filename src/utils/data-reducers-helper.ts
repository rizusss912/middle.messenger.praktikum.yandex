...
export class DataReducersHelper {
    ...
    public static createUploadedReducer<state, action extends Action>(field: keyof state): reducer<state> {
    ...
    public static createUploadErrorReducer<state, action extends Action>(field: keyof state): reducer<state> {
        return (state: state, action: action) =>
            ({
                ...state,
                [field]: {

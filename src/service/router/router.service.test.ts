...
describe('service: RouterService', () => {
	...
	describe('$queryParams', () => {
		...
		it('is not gets value if the params has not changed', async () => {
			...
			const {callCount} = queryParamsStub;
		...
		it('custom value', async () => {
			...
			expect(
				queryParamsStub.lastCall
					.calledWith({firstParam: firstQueryParamsValue.secondValue}),
			).toBe(true);
		});
	});
